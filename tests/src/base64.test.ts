...
describe("test Base64", () => {
    test("encode then decode (raw)", () => {
        const result = Base64.decode(Base64.encode(sourceHexString));
        expect(result).toEqual(sourceHexString);
    });

    test("encode then decode (string)", () => {
        const result = Base64.toString(Base64.fromString(sourceString));
        expect(result).toEqual(sourceString);
    });

    test("encode then decode (hex)", () => {
        const result = Base64.toHex(Base64.fromHex(sourceHexString));
        expect(result).toEqual(sourceHexString);
    });

    test("encode then decode (url format)", () => {
        const result = Base64.toString(Base64.fromUrlFormat(Base64.toUrlFormat(Base64.fromString(sourceUrlString))));
